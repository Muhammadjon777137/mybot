...
BOT_TOKEN = "7293317721:AAGWnPYYGGtZEdDPUFy0pYLDwMKSwdC91u8"
...
async def start_bot(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await message.answer(
            ...
            f"Powered by @Dominatexxxx", reply_markup=back())
        await state.finish()
...
async def download_youtube_video(message: types.Message):
    ...
    try:
        ...
        await message.reply_video(video=open(video_path, 'rb'), caption="Powered by @Dominatexxxx")
...
async def send_username(message: types.Message):
    ...
    if len(username) == 13:
        ...
        await message.answer("Raqamni notogri kiritngiz")
